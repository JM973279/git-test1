...
def _learn_document_map() -> dict[str, Path]:
    """Expose the ten curriculum units and vocabulary support documents."""
    return {
        path.stem: path
        for path in DOCS_DIR.glob("*.md")
        if re.fullmatch(r"[A-Za-z0-9_-]+", path.stem)
        and (path.stem.startswith("unit-") or path.stem in {"voca", "voca-exam"})
    }
...
app = FastAPI(
    title="JUMI Finance Lab API",
    version="1.0.0",
    description=(
        "10단원 금융지식 학습·RAG 검색·투자분석·백테스트 통합 API"
    ),
...
def health_check() -> dict[str, str]:
    return {"status": "ok", "service": "jumi-finance-lab", "version": "1.0.0"}
