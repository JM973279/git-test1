...
function navigate(view) {
  ...
  const gnbGroup = view === 'home'
    ? 'home'
    : view?.startsWith('learn-')
      ? 'curriculum'
      : view === 'rag-chat'
        ? 'rag'
        : ['quant-lean', 'backtest', 'pipeline', 'risk'].includes(view)
          ? 'backtest'
          : 'analysis';
  document.querySelectorAll('.gnb-item[data-gnb]').forEach(button => {
    button.classList.toggle('active', button.dataset.gnb === gnbGroup);
  });
  ...
  if (view?.startsWith('learn-')) activeSections.push('curriculum');
