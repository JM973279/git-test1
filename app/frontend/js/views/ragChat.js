const EXAMPLES = ['금리 상승이 채권가격에 미치는 영향을 설명해 줘', 'ETF 추적오차와 괴리율의 차이는?', 'VaR와 스트레스 테스트를 비교해 줘', '백테스트 정보누수 체크리스트를 만들어 줘'];
    ...
    app.innerHTML = `
      <section class="card rag-chat-heading">
        <div class="rag-chat-heading-copy">
          <h2><i class="fa-solid fa-comments"></i>나만의 금융지식 RAG</h2>
          <p>10개 금융 단원에서 관련 근거를 찾아 답합니다. 답변 아래의 검색 원문과 출처를 반드시 함께 확인하세요.</p>
        </div>
      ...
      <section class="rag-chat-layout rag-answer-layout">
        <section class="card rag-chat-panel">
          <div id="rag-messages" class="rag-messages" aria-live="polite">
            ${messages.length ? messages.map((message) => `<div class="rag-message is-${message.role}">${formatText(message.text)}</div>`).join('') : '<div class="rag-welcome"><strong>10개 단원에서 무엇을 찾아볼까요?</strong><br>예: “듀레이션과 만기의 차이를 설명해 줘.”</div>'}
          </div>
